...
public class SOSInboundTransport extends InboundTransportBase implements Runnable {
	...
	private void receiveData() {
		...
		try {
			...
			//Allocate ByteBuffer with given size
			bb = ByteBuffer.allocate(data.length);
			//Transfer the content of the byte array into the buffer
			bb.put(data);
			//Prepares the buffer for transferring
			bb.flip();
			byteListener.receive(bb, "");
			//Reset buffer for following uses
			bb.clear();
